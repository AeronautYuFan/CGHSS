...
# Load the full dataset from Airtable
fullDataSet = pd.read_csv(file_utils.get_airtable())

# Include the "Powers Invoked" column in the cleaned dataset
cleaned_dataSet = fullDataSet[['Citation', 'Entity Empowered', 'Triggering Event', 'Location', 'Powers Invoked']]
...
for index, row in cleaned_dataSet.iterrows():
    ...
    for agency in agencies:
        ...
        if agency not in agencies_processed:
            ...
            filtered_data = cleaned_dataSet[cleaned_dataSet['Entity Empowered'].str.contains(pattern, na=False, regex=True)].copy()

            # Add the agency name as a new column with a space in the header
            filtered_data = filtered_data.assign(**{"Specific Agency": agency})
